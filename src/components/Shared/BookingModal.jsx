    ...
    <div className="mx-auto flex min-h-full max-w-3xl items-center" onClick={(event) => event.stopPropagation()}>
      <div className="relative my-4 w-full overflow-hidden rounded-2xl border border-[#D4AF37]/30 bg-[#07172E] shadow-2xl">
        ...
        <form onSubmit={submit} className="px-5 py-6 sm:px-9 sm:py-8">
          ...
          <div className="mt-6 grid gap-3 border-t border-white/10 pt-5 sm:grid-cols-2"><a href={`tel:${settings.phone || ""}`} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3 !text-[#D4AF37] hover:border-[#D4AF37]/50"><Phone className="text-[#D4AF37]" size={18} /><span><small className="block text-slate-400">Call directly</small><strong className="text-sm">{settings.phone || "Phone unavailable"}</strong></span></a><a href={`mailto:${settings.email || ""}`} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3 !text-[#D4AF37] hover:border-[#D4AF37]/50"><Mail className="text-[#D4AF37]" size={18} /><span className="min-w-0"><small className="block text-slate-400">Email directly</small><strong className="block truncate text-sm">{settings.email || "Email unavailable"}</strong></span></a></div>
        </form>
      </div>
    </div>
  </div>;
